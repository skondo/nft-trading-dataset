-- Create weekly collection returns together with market and FX returns.
...
CREATE OR REPLACE TABLE `%s.%s.weekly_returns_vw` AS
WITH nft_ret AS (
  SELECT
    collection,
    week_start,
    ...
    CASE
      WHEN LAG(week_start) OVER (PARTITION BY collection ORDER BY week_start) IS NOT NULL
       AND DATE_DIFF(week_start, LAG(week_start) OVER (PARTITION BY collection ORDER BY week_start), DAY) = 7
       AND median_price_usd > 0
       AND LAG(median_price_usd) OVER (PARTITION BY collection ORDER BY week_start) > 0
      THEN LOG(SAFE_DIVIDE(median_price_usd, LAG(median_price_usd) OVER (PARTITION BY collection ORDER BY week_start)))
      ELSE NULL
    END AS nft_return
  FROM `%s.%s.weekly_collection_panel_filtered`
),
market_ret AS (
  SELECT
    week_start,
    SAFE_DIVIDE(SUM(nft_return * total_volume_usd), SUM(total_volume_usd)) AS market_return
  FROM nft_ret
...
fx_ret AS (
  SELECT
    week_start,
    LOG(SAFE_DIVIDE(avg_usd_eth_rate, LAG(avg_usd_eth_rate) OVER (ORDER BY week_start))) AS fx_return
  FROM fx_weekly
)
SELECT
  n.collection,
...
""", PROJECT_ID, ANALYTICS_DATASET_ID, PROJECT_ID, ANALYTICS_DATASET_ID, PROJECT_ID, BASE_DATASET_ID);
